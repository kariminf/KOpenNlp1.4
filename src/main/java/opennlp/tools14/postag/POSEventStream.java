...
public class POSEventStream implements EventStream {
  ...
  public static void main(String[] args) throws java.io.IOException, InvalidFormatException {
    EventStream es;
    if (args.length == 0) {
      es = new POSEventStream(new opennlp.maxent25.PlainTextByLineDataStream(new java.io.InputStreamReader(System.in)));
    }
    else {
      es = new POSEventStream(new opennlp.maxent25.PlainTextByLineDataStream(new java.io.InputStreamReader(System.in)),new Dictionary(new FileInputStream(args[0])));
    }
    while (es.hasNext()) {
