...
//////////////////////////////////////////////////////////////////////////////   
package opennlp.tools14.parser.treeinsert;
...
import opennlp.maxent25.TwoPassDataIndexer;
import opennlp.tools14.parser.AbstractBottomUpParser;
import opennlp.tools14.parser.HeadRules;
import opennlp.tools14.parser.Parse;
import opennlp.tools14.parser.ParserChunker;
import opennlp.tools14.parser.ParserEventTypeEnum;
import opennlp.tools14.parser.ParserTagger;
...
public class Parser extends AbstractBottomUpParser {
  ...
  public static GISModel train(opennlp.maxent25.EventStream es, int iterations, int cut) throws java.io.IOException {
    return opennlp.maxent25.GIS.trainModel(iterations, new TwoPassDataIndexer(es, cut));
  }
  ...
  public static void main(String[] args) throws java.io.IOException {
    ...
    HeadRules rules = new opennlp.tools14.lang.english.HeadRules(modelDirectory+"/head_rules");
    ...
    if (tag || all) {
      ...
      opennlp.maxent25.EventStream tes = new ParserEventStream(new opennlp.maxent25.PlainTextByLineDataStream(new java.io.FileReader(inFile)), rules, ParserEventTypeEnum.TAG);
      GISModel tagModel = train(tes, iterations, cutoff);
      System.out.println("Saving the tagger model as: " + tagFile);
      new opennlp.maxent25.io.SuffixSensitiveGISModelWriter(tagModel, tagFile).persist();
    }

    if (chunk || all) {
      System.err.println("Training chunker");
      opennlp.maxent25.EventStream ces = new ParserEventStream(new opennlp.maxent25.PlainTextByLineDataStream(new java.io.FileReader(inFile)), rules, ParserEventTypeEnum.CHUNK);
      GISModel chunkModel = train(ces, iterations, cutoff);
      System.out.println("Saving the chunker model as: " + chunkFile);
      new opennlp.maxent25.io.SuffixSensitiveGISModelWriter(chunkModel, chunkFile).persist();
    }
        
    if (build || all) {
      System.err.println("Training builder");
      opennlp.maxent25.EventStream bes = new ParserEventStream(new opennlp.maxent25.PlainTextByLineDataStream(new java.io.FileReader(inFile)), rules, ParserEventTypeEnum.BUILD,null);
      GISModel buildModel = train(bes, iterations, cutoff);
      System.out.println("Saving the build model as: " + buildFile);
      new opennlp.maxent25.io.SuffixSensitiveGISModelWriter(buildModel, buildFile).persist();
    }

    if (attach || all) {
      System.err.println("Training attacher");
      opennlp.maxent25.EventStream kes = new ParserEventStream(new opennlp.maxent25.PlainTextByLineDataStream(new java.io.FileReader(inFile)), rules, ParserEventTypeEnum.ATTACH);
      GISModel attachModel = train(kes, iterations, cutoff);
      System.out.println("Saving the attach model as: " + attachFile);
      new opennlp.maxent25.io.SuffixSensitiveGISModelWriter(attachModel, attachFile).persist();
    }
    
    if (check || all) {
      System.err.println("Training checker");
      opennlp.maxent25.EventStream ces = new ParserEventStream(new opennlp.maxent25.PlainTextByLineDataStream(new java.io.FileReader(inFile)), rules, ParserEventTypeEnum.CHECK);
      GISModel checkModel = train(ces, iterations, cutoff);
      System.out.println("Saving the check model as: " + checkFile);
      new opennlp.maxent25.io.SuffixSensitiveGISModelWriter(checkModel, checkFile).persist();
    }
  }
}
