...
public class DefaultNameContextGenerator implements NameContextGenerator {
  ...
  private static AdaptiveFeatureGenerator staticFeatures = new CachedFeatureGenerator(
  ...
  public String[] getContext(int index, Object[] toks, String[] preds, String[][] additionalContext) {
    ...
    if (prevTokens == toks) {
      tokens = prevStrings;
      if (index == cpi && po.equals(cpo) && ppo.equals(cppo)) {
        //pc++;
    ...
    prevContext = (String[]) features.toArray(new String[features.size()]);
    cpi=index;
    cpo = po;
    cppo = ppo;
  ...
  private void cacheReport() {
    System.err.println(callNum+" "+staticFeatures.toString()+" pc="+pc+" "+(double) pc/callNum);
  }
}
